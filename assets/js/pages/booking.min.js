...
App.Pages.Booking = (function () {
    ...
    function addEventListeners() {
        ...
        $(document).on('click', '.js-category-toggle', function () {
            ...
            const $body = $('#' + targetId);
            $body.toggleClass('ea-collapsed');

            const $caret = $(this).find('.category-caret');
            if ($caret.length) {
                $caret.text($body.hasClass('ea-collapsed') ? '▸' : '▾');
            }
        });
    ...
    /**
     * Возвращает множество ID услуг, которые умеют все переданные провайдеры (объединение их услуг).
     */
    function getAllowedServiceIdsFromProviders(providers) {
        const allowed = new Set();

        (providers || []).forEach((provider) => {
            getProviderServiceIds(provider).forEach((sid) => allowed.add(String(sid)));
        });
